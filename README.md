...
- **Tests**: use Node's built-in runner via tsx — `node --import tsx --test test`. Test files live under `test/` and use `.js` import extensions (per NodeNext) — tsx resolves them to the actual `.ts` sources at runtime.
